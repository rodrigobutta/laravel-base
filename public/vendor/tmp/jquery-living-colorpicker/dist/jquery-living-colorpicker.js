...
(function($) {
  ...
  $.fn.livingColorPicker = function(options) {
    ...
    return this.each(function() {
      ...
      function init() {
        ...
        rendermode = $.fn.livingColorPicker.detectWhichGradientIsSupported();

        if (rendermode === 'filter') {
          rendermode = false;
        }

        if (!rendermode && $.fn.livingColorPicker.svgSupported()) {
          rendermode = 'svg';
        }
      ...
      function _initColor() {
        ...
        color.cielch = $.fn.livingColorPicker.rgb2lch(color.rgba);
      }
      ...
      function _getControllerHtml() {
        ...
        if (settings.grouping) {
          ...
          if (settings.swatches) {
            color_picker_html += '<li><a href="#" class="cp-pill-swatches">Predefinidos</a></li>';
          }
          if (settings.hsvpanel) {
            color_picker_html += '<li><a href="#" class="cp-pill-hsvpanel">Personalizado</a></li>';
          }
      ...
      function activateLastlyUsedGroup() {
        ...
        switch (getLastlyUsedGroup()) {
          ...
          default:
             ...
             if (elements.pills.sliders.length) {
              activateGroupSliders();
              break;
            }
            else if (elements.pills.hsvpanel.length) {
              activateGroupHsvpanel();
              break;
            }
      ...
      function _updateColorsProperty(format, property, value) {
        switch (format) {
          ...
          case 'cielch':
            ...
            color.rgba = $.fn.livingColorPicker.lch2rgb(color.cielch);
      ...
      function _updateMarkerPosition(slidername, ev) {
        var percent = $.fn.livingColorPicker.calculateEventPositionPercentage(ev, elements.sliders[slidername]);
      ...
      function _updateHsvpanelMarkerPosition(marker, ev) {
        var percents = $.fn.livingColorPicker.calculateEventPositionPercentage(ev, elements.hsvpanel.sv, true);
      ...
      function _renderHsva() {
        setGradient(elements.hsvpanel.a, $.fn.livingColorPicker.getScaledGradientStops(color.hsla, 'a', 1, 0, 2), true);

        elements.hsvpanel.a_marker.css('top', 100 - color.hsv.a * 100 + '%');
      }

      function _renderHue() {
        setGradient(elements.sliders.hue, $.fn.livingColorPicker.getScaledGradientStops(color.hsla, 'h', 0, 360, 7));

        elements.sliders.hue_marker.css('left', color.hsla.h / 360 * 100 + '%');
      }

      function _renderSaturation() {
        setGradient(elements.sliders.saturation, $.fn.livingColorPicker.getScaledGradientStops(color.hsla, 's', 0, 1, 2));

        elements.sliders.saturation_marker.css('left', color.hsla.s * 100 + '%');
      }

      function _renderLightness() {
        setGradient(elements.sliders.lightness, $.fn.livingColorPicker.getScaledGradientStops(color.hsla, 'l', 0, 1, 3));

        elements.sliders.lightness_marker.css('left', color.hsla.l * 100 + '%');
      }

      function _renderOpacity() {
        setGradient(elements.sliders.opacity, $.fn.livingColorPicker.getScaledGradientStops(color.hsla, 'a', 0, 1, 2));

        elements.sliders.opacity_marker.css('left', color.hsla.a * 100 + '%');
      }

      function _renderRed() {
        setGradient(elements.sliders.red, $.fn.livingColorPicker.getScaledGradientStops(color.rgba, 'r', 0, 255, 2));

        elements.sliders.red_marker.css('left', color.rgba.r / 255 * 100 + '%');
      }

      function _renderGreen() {
        setGradient(elements.sliders.green, $.fn.livingColorPicker.getScaledGradientStops(color.rgba, 'g', 0, 255, 2));

        elements.sliders.green_marker.css('left', color.rgba.g / 255 * 100 + '%');
      }

      function _renderBlue() {
        setGradient(elements.sliders.blue, $.fn.livingColorPicker.getScaledGradientStops(color.rgba, 'b', 0, 255, 2));
        ...
        for (var i = 1; i < gradientstops.length; i++) {
          if (gradientstops[i].isok !== gradientstops[i - 1].isok) {
            var steps = Math.round(gradientstops[i].position) - Math.round(gradientstops[i - 1].position),
                extendedgradientstops = $.fn.livingColorPicker.getScaledGradientStops(gradientstops[i].rawcolor, property, gradientstops[i - 1].rawcolor[property], gradientstops[i].rawcolor[property], steps, settings.invalidcolorsopacity, gradientstops[i - 1].position, gradientstops[i].position);
      ...
      function _renderCieLightness() {
        var gradientstops = $.fn.livingColorPicker.getScaledGradientStops(color.cielch, 'l', 0, 100, 10, settings.invalidcolorsopacity);
      ...
      function _renderCieChroma() {
        var gradientstops = $.fn.livingColorPicker.getScaledGradientStops(color.cielch, 'c', 0, MAXVALIDCHROMA, 5, settings.invalidcolorsopacity);
      ...
      function _renderCieHue() {
        var gradientstops = $.fn.livingColorPicker.getScaledGradientStops(color.cielch, 'h', 0, 360, 28, settings.invalidcolorsopacity);
      ...
      function _renderPreview() {
        elements.sliders.preview.css('background', $.fn.livingColorPicker.csscolor(color.rgba));
      ...
      function setGradient(element, gradientstops, vertical) {
        ...
        switch (rendermode) {
          case 'noprefix':
            $.fn.livingColorPicker.renderNoprefix(element, gradientstops, vertical);
            break;
          case 'webkit':
            $.fn.livingColorPicker.renderWebkit(element, gradientstops, vertical);
            break;
          case 'ms':
            $.fn.livingColorPicker.renderMs(element, gradientstops, vertical);
            break;
          case 'svg': // can not repeat, radial can be only a covering ellipse (maybe there is a workaround, need more investigation)
            $.fn.livingColorPicker.renderSVG(element, gradientstops, vertical);
            break;
          case 'oldwebkit':   // can not repeat, no percent size with radial gradient (and no ellipse)
            $.fn.livingColorPicker.renderOldwebkit(element, gradientstops, vertical);
            break;
        }
      }
  ...
  $.fn.livingColorPicker.getEventCoordinates = function(ev) {
  ...
  $.fn.livingColorPicker.calculateEventPositionPercentage = function(ev, containerElement, both) {
    if (typeof (both) === 'undefined') {
      both = false;
    }

    var c = $.fn.livingColorPicker.getEventCoordinates(ev);
  ...
  $.fn.livingColorPicker.getScaledGradientStops = function(color, scalableproperty, minvalue, maxvalue, steps, invalidcolorsopacity, minposition, maxposition) {
    ...
    for (var i = 0; i < steps; ++i) {
      var currentstage = i / (steps - 1),
          modifiedcolor = $.fn.livingColorPicker.modifyColor(color, scalableproperty, currentstage * diff + minvalue),
          csscolor;

      if (invalidcolorsopacity < 1) {
        var stagergb = $.fn.livingColorPicker.lch2rgb(modifiedcolor, invalidcolorsopacity);

        isok = stagergb.isok;
        csscolor = $.fn.livingColorPicker.csscolor(stagergb, invalidcolorsopacity);
      }
      else {
        csscolor = $.fn.livingColorPicker.csscolor(modifiedcolor, invalidcolorsopacity);
      }
  ...
  $.fn.livingColorPicker.getGradientStopsCSSString = function(gradientstops) {
    var gradientstring = '',
        oldwebkit = '',
        svgstoppoints = '';
  ...
  $.fn.livingColorPicker.renderNoprefix = function(element, gradientstops, vertical) {
    if (typeof vertical === 'undefined') {
      vertical = false;
    }

    var css,
        stoppoints = $.fn.livingColorPicker.getGradientStopsCSSString(gradientstops).noprefix;
  ...
  $.fn.livingColorPicker.renderWebkit = function(element, gradientstops, vertical) {
    if (typeof vertical === 'undefined') {
      vertical = false;
    }
  ...
  $.fn.livingColorPicker.renderOldwebkit = function(element, gradientstops, vertical) {
    if (typeof vertical === 'undefined') {
      vertical = false;
    }

    var css,
        stoppoints = $.fn.livingColorPicker.getGradientStopsCSSString(gradientstops).oldwebkit;
  ...
  $.fn.livingColorPicker.renderMs = function(element, gradientstops, vertical) {
    if (typeof vertical === 'undefined') {
      vertical = false;
    }
  ...
  $.fn.livingColorPicker.renderSVG = function(element, gradientstops, vertical) {
    if (typeof vertical === 'undefined') {
      vertical = false;
    }

    var svg = '',
        svgstoppoints = $.fn.livingColorPicker.getGradientStopsCSSString(gradientstops).svg;
    ...
    svg = 'url(data:image/svg+xml;base64,' + $.fn.livingColorPicker.base64encode(svg) + ')';

    element.css('background-image', svg);
  };

  /* source: http://phpjs.org/functions/base64_encode/ */
  $.fn.livingColorPicker.base64encode = function(data) {
  ...
  $.fn.livingColorPicker.isGoodRgb = function(rgb) {
  ...
  $.fn.livingColorPicker.rgb2lch = function(rgb) {
    var lch = $.fn.livingColorPicker.CIELab2CIELCH($.fn.livingColorPicker.XYZ2CIELab($.fn.livingColorPicker.rgb2XYZ(rgb)));
  ...
  $.fn.livingColorPicker.lch2rgb = function(lch, invalidcolorsopacity) {
    if (typeof invalidcolorsopacity === 'undefined') {
      invalidcolorsopacity = 1;
    }

    var rgb = $.fn.livingColorPicker.XYZ2rgb($.fn.livingColorPicker.CIELab2XYZ($.fn.livingColorPicker.CIELCH2CIELab(lch)));

    if ($.fn.livingColorPicker.isGoodRgb(rgb)) {
    ...
    do {
      ...
      rgb = $.fn.livingColorPicker.XYZ2rgb($.fn.livingColorPicker.CIELab2XYZ($.fn.livingColorPicker.CIELCH2CIELab(tmp)));
  ...
  $.fn.livingColorPicker.modifyColor = function(color, property, value) {
  ...
  $.fn.livingColorPicker.csscolor = function(color, invalidcolorsopacity) {
    if (typeof invalidcolorsopacity === 'undefined') {
      invalidcolorsopacity = 1;
    }
    ...
    if (tmpcolor.hasOwnProperty('c')) {
      // CIE-LCh
      tmpcolor = $.fn.livingColorPicker.lch2rgb(tmpcolor, invalidcolorsopacity);
    }
  ...
  $.fn.livingColorPicker.rgb2XYZ = function(rgb) {
    var XYZ = {};
  ...
  $.fn.livingColorPicker.XYZ2CIELab = function(XYZ) {
    var CIELab = {};
  ...
  $.fn.livingColorPicker.CIELab2CIELCH = function(CIELab) {
    var CIELCH = {};
  ...
  $.fn.livingColorPicker.CIELCH2CIELab = function(CIELCH) {
    var CIELab = {};
  ...
  $.fn.livingColorPicker.CIELab2XYZ = function(CIELab) {
    var XYZ = {};
  ...
  $.fn.livingColorPicker.XYZ2rgb = function(XYZ) {
    var rgb = {};
  ...
  $.fn.livingColorPicker.detectWhichGradientIsSupported = function() {
  ...
  $.fn.livingColorPicker.svgSupported = function() {
