...
(function(){
    $AEE.ready(function(){
        ...
        $AEE.settings.tinymceBlock = {
            ...
            toolbar: [
                "styleselect | undo redo | alignleft aligncenter alignright alignjustify | image | link | bullist numlist",
                "bold italic underline | fontselect fontsizeselect | forecolor backcolor | table | code",
                "customfields systemfields imageupload",
            ],
            ...
            setup: function (editor) {
                ...
                inp.on("change",function(){
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            ...
                            currentEditor.focus();
                            currentEditor.selection.moveToBookmark(currentEditorBookmark);
                            currentEditor.insertContent('<a href="'+data.url+'"/>'+data.name+'</a>');
                ...
                //         currentEditorBookmark = editor.selection.getBookmark();
                //         inp.trigger('click');
                //     }
                // });
                ...
                //     type: 'menubutton',
                //     text: $A.translate('Insertar'),
                //     // icon: 'none fa fa-user',
                //     menu: [
                //         {
                //             text: $A.translate('Usuario'),
                //             menu: $AEE.getTinyMceUserDefinedMenu(editor)
                //         },
                //         {
                //             text: $A.translate('Archivo'),
        ...
        $AEE.settings.tinymceHtmlBlock = {
            ...
            setup: function (editor) {
                ...
                editor.addButton('automizyImage', {
                    icon: 'image',
                    tooltip: $A.translate('Seleccionar imagen'),
                    onclick: function() {
                        var dom = editor.dom;
                        ...
                        if (imgElm.nodeName == 'IMG' && !imgElm.getAttribute('data-mce-object') && !imgElm.getAttribute('data-mce-placeholder')) {
                            ...
                            $AEE.imagePicker
                                .reset()
                                .dialogTitle($A.translate('Modificar imagen'))
    ...
    $AEE.layoutReady(function(){
        var ImagePicker = function (obj) {
            ...
            t.d.buttons.delete = $A.newButton({
                text: $A.translate('Eliminar imagen'),
                click: function () {
                    t.delete();
                }
            });
            ...
            t.d.inputs.upload.input().change(function(){
                ...
                complete: function () {
                    t.d.inputs.upload.data('automizyButton').enable().text($A.translate('Imagen desde la computadora..'));
            ...
            t.d.inputs.url = $A.newInput({
                placeholder:$A.translate('Url de la imagen'),
                target:t.d.$controlImageBox,
                type:'text',
                labelWidth: 'auto',
                breakInput: false,
            ...
            t.d.inputs.link = $A.newInput({
                ...
                help:$A.translate('Si al hacer click en la imagen, deba dirigir el usuario a otra página.'),
                value:'http://',
                change:function(){
            ...
            t.d.inputs.alt = $A.newInput({
                label:$A.translate('Texto alternativo'),
                help:$A.translate('Texto que aparece si la imagen no pudo ser mostrada por algun motivo. Es requerido.'),
                change:function(){
                    t.d.img.alt = this.val();
                }
            });
            t.d.inputs.title = $A.newInput({
                label:$A.translate('Título de la imagen'),
                help:$A.translate('Texto que aparece cuando el usuario se para sobre la imagen.'),
                change:function(){
                    t.d.img.title = this.val();
                }
            });
            t.d.inputs.align = $A.newInput({
                label:$A.translate('Alineación'),
                help:$A.translate('Alineación horizontal'),
                type:'select',
                options:[
                    ['left', $A.translate('Izquierda')],
                    ['center', $A.translate('Centrado'), true],
                    ['right', $A.translate('Derecha')]
                ],
                change:function(){
            ...
            t.d.inputs.minWidth = $A.newInput({
                label:$A.translate('Ancho mínimo'),
                help:$A.translate('El ancho mínimo de la imagen (en pixels)'),
                change:function(){
        ...
        p.url = p.src = function (value) {
            ...
            if (typeof value !== 'undefined') {
                if(value === '' || typeof value !== 'string') {
                    ...
                    t.d.inputs.upload.data('automizyButton').text($A.translate('Explorar en la computadora..'));
                }else{
                    t.d.img.src = value;
    ...
    $AEE.ready(function(){
        ...
        $AEE.blocks.push({
            ...
            drop:function($block, $contentCell, $topCell, $rightCell, $bottomCell, $leftCell){
                ...
                var $selectImageButton = $('<a href="javascript:;" class="aee-image-block-button"></a>').text($A.translate('Seleccionar imagen')).appendTo($imageBox);
                $AEE.imagePicker
                    .reset()
                    .save(function(img){
                        if(img.$elem !== false){
...
(function(){
    ...
    $AEE.ready(function() {
        ...
        $AEE.inputs.buttonSettings.text = $A.newInput({
            label:$A.translate('Texto del botón'),
            value:$A.translate('Botón'),
            validator:'notEmpty',
            change:function(){
                styleClone();
            }
        ...
        $AEE.inputs.buttonSettings.paddingTop = $A.newInput({
            label:$A.translate('Espacio superior'),
            type:'number',
            labelAfter:' px',
            create:function(){
                this.input().attr('min', 0).pbmInput();
        ...
        $AEE.inputs.buttonSettings.paddingRight = $A.newInput({
            label:$A.translate('Espacio a derecha'),
            type:'number',
            labelAfter:' px',
            create:function(){
                this.input().attr('min', 0).pbmInput();
        ...
        $AEE.inputs.buttonSettings.paddingBottom = $A.newInput({
            label:$A.translate('Espacio inferior'),
            type:'number',
            labelAfter:' px',
            create:function(){
                this.input().attr('min', 0).pbmInput();
        ...
        $AEE.inputs.buttonSettings.paddingLeft = $A.newInput({
            label:$A.translate('Espacio a izquierda'),
            type:'number',
            labelAfter:' px',
            create:function(){
                this.input().attr('min', 0).pbmInput();
        ...
        $AEE.inputs.buttonSettings.textPosition = $A.newInput({
            label:$A.translate('Alineación del texto'),
            type:'select',
            options:[
                ['left', $A.translate('Izquierda')],
                ['center', $A.translate('Centrado'), true],
        ...
        $AEE.inputs.buttonSettings.fontFamily = $A.newInput({
            label:$A.translate('Tipo de Fuente'),
            type:'select',
        ...
        $AEE.inputs.buttonSettings.backgroundColor = $A.newInput({
            label:$A.translate('Color de fondo'),
            width:'32px',
            create:function(){
                this.input().css({
                    fontSize:0,
        ...
        $AEE.inputs.buttonSettings.textColor = $A.newInput({
            label:$A.translate('Color de texto'),
            width:'32px',
            create:function(){
                this.input().css({
                    fontSize:0,
        ...
        $AEE.dialogs.buttonSettings = $A.newDialog({
            title:$A.translate('Configuración del botón'),
            content:$table,
            width:'85%',
            buttons:[
...
(function(){
    ...
    $AEE.layoutReady(function(){
        ...
        $AEE.addEmptyGalleryItem = function($contentCell){
            ...
            var $selectImageButton = $('<a href="javascript:;" class="aee-gallery-block-button"></a>').text($A.translate('Seleccionar imagen')).appendTo($imageBox);
        };
        ...
        $AEE.inputs.blockSettingsGalleryDistanceBetween = $A.newInput({
            type:'number',
            label:$A.translate('Espaciado entre imagenes'),
            labelAfter:'px',
            value:6,
            enter:function(){
                $AEE.rebuildGallery();
...
(function(){
    ...
    $AEE.layoutReady(function(){
        ...
        $AEE.inputs.blockSettingsColumns1 = $A.newInput({
            type:'checkbox',
            labelWidth:'150px',
            label:$A.translate('Primer columna'),
            newRow:false,
            checked:true,
            change:function(){
                $AEE.rebuildColumns();
        ...
        $AEE.inputs.blockSettingsColumns1MinWidth = $A.newInput2({
            type:'number',
            labelBefore:$A.translate('Ancho mínimo:'),
            labelAfter:'px',
            value:'200',
            create:function(){
                this.input().pbmInput();
        ...
        $AEE.inputs.blockSettingsColumns2 = $A.newInput({
            type:'checkbox',
            labelWidth:'150px',
            label:$A.translate('Segunda columna'),
            checked:true,
            change:function(){
                $AEE.rebuildColumns();
            }
        ...
        $AEE.inputs.blockSettingsColumns3 = $A.newInput({
            type:'checkbox',
            labelWidth:'150px',
            label:$A.translate('Tercer columna'),
            checked:false,
            change:function(){
                $AEE.rebuildColumns();
            }
        ...
        $AEE.inputs.blockSettingsColumns4 = $A.newInput({
            type:'checkbox',
            labelWidth:'150px',
            label:$A.translate('Cuarta columna'),
            checked:false,
            change:function(){
                $AEE.rebuildColumns();
            }
...
(function(){
    ...
    $AEE.setBlockSettings = function ($block) {
        ...
        if($block.attr('id') === 'aee-document'){
            ...
            $AEE.elements.$blockSettingsContentTitle.html($A.translate('Configuración General'));
        }else {
            $block.addClass('aee-active');
            ...
            $AEE.elements.$blockSettingsContentTitle.html($A.translate('Configuración del elemento'));
        }
...
(function(){
    $AEE.ready(function () {
        ...
        $AEE.elements.$widget = $('<div id="automizy-email-editor"></div>').appendTo('#email_editor');
        if ($AEE.touchable()) {
        ...
        $AEE.inputs.blockSettingsResponsiveEmail = $A.newInput2({
            type: 'checkbox',
            labelBefore: $A.translate('Mail Responsivo'),
            checked: true,
            change: function () {
                if (this.checked()) {
                    $AEE.inputs.blockSettingsDocumentMaxWidth.labelBefore($A.translate('Ancho Máximo'));
                    $AEE.elements.$document.attr('data-responsive-email', '1');
                } else {
                    $AEE.inputs.blockSettingsDocumentMaxWidth.labelBefore($A.translate('Ancho'));
        ...
        $AEE.inputs.blockSettingsDocumentMaxWidth = $A.newInput2({
            type: 'number',
            labelBefore: $A.translate('Ancho Máximo'),
            value: 800,
            enter: function () {
                this.change();
                return false;
        ...
        $AEE.inputs.blockSettingsDocumentOuterColor = $A.newInput2({
            type: 'text',
            labelBefore: $A.translate('Color de fondo'),
            change: function () {
        ...
        $('<style>#aee-document .aee-ui-state-highlight:before{content: "' + $A.translate("Soltar acá") + '"}</style>').appendTo('body:first');
        $('<style>#aee-block-list .aee-ui-state-highlight:before{content: "' + $A.translate("Elimiar") + '"}</style>').appendTo('body:first');
