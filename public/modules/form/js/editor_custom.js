...
var EditorViewModel = function(data) {
 ...
 this.selectFieldAtIndex = function(index) {
	...
	this.addField = function(data, event) {
	 ...
	 var schema = $(event.target).data("schema");
	 data.schema = schema; //JSON.parse(schema);

	 var nature = $(event.target).data("nature");
	 data.nature = nature;
...
var FormViewModel = function(data) {
	...
	this.addField = function(data) {
		...
		if(data.schema){
			...
			schema = getDefaultDataForType(data.type);
		}

		ko.mapping.fromJS(schema, {}, newField);
...
owner: this
	...
	this.hasNature = ko.computed(function() {
